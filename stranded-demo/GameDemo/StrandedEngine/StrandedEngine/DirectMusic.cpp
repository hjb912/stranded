...
bool CDMSoundObject::IsPlaying()
{
   if(!m_audioSound) return false;

   // If it is already playing, return.
   if(m_soundPerformance->IsPlaying(m_audioSound,
      NULL) == S_OK) return true;
...
bool CDirectMusicSystem::SetupSoundParameters(int id,
                           ...
                           float min, float max)
{
   if(id < 0 || id >= m_totalSounds) return false;

   if(!m_soundList) return false;
...
bool CDirectMusicSystem::IsPlaying(int id)
{
   if(id < 0 || id >= m_totalSounds)
      return false;
...
{
   if(id < 0 || id >= m_totalSounds) return;
